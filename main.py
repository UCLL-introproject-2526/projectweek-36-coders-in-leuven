...
def update_player_delay(player):
    DELAY_TIME = 2000  
    current_time = pygame.time.get_ticks()

    if not player.canMove:
        if current_time - player.spawn_time >= DELAY_TIME:
            player.canMove = True
...
def restart_game(player, car_manager, train_manager):
    ...
    player.spawn_time = pygame.time.get_ticks()
...
def main():
    ...
    while running:
        ...
        update_player_delay(player)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
            
            if event.type == KEYDOWN and player.canMove:
                if event.key == K_p:
        ...
        if not player.canMove:
            ...
            screen.blit(text, (WIDTH//2 - 50, 40))
